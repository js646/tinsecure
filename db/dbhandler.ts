...
const db = new sqlite3.Database('./db/database.db', sqlite3.OPEN_READWRITE, (err: Error) => {
    if(err)
...
db.serialize( () => {
    ...
    db.run(`INSERT INTO users(lastname, firstname, birthday, email, password, sex, city, description, picture, secAnswer)
            ...
            ('Dude', 'The', '1966-12-04', 'thedude@dude.de', '${crypto.SHA1('123456')}', 'm', 'Venice, California', 'a single, unemployed man whose hobbies include smoking pot, drinking White Russians, and bowling', 'https://static.wikia.nocookie.net/thebiglebowski/images/7/7e/The_Dude.jpeg/revision/latest?cb=20111216183045', 'pot'),
    ...
    db.run(`INSERT INTO matches(uid1, uid2, chat_file)
            ...
            (1, 2, 'chats/chat_1-2'),
            (3, 4, 'chats/chat_3-4'),
            (6, 10, 'chats/chat_6-10'),
            (12, 7, 'chats/chat_3-4'),
            (1, 2, 'chats/chat_1-2'),
            (7, 9, 'chats/chat_7-9'),
            (5, 10, 'chats/chat_5-10'),
            (9, 11, 'chats/chat_9-13'),
            (8, 12, 'chats/chat_8-12')
            `)
...
module.exports.createMatch = function(uid1:number, uid2:number):void{
    fs.stat(`./chats/chat_${uid1}-${uid2}`, function(err, stat) {
        if(err == null) {
            // file already exists
            return;
        } else if(err.code === 'ENOENT') {
            // file does not exist => create it
            fs.open(`./chats/chat_${uid1}-${uid2}`, "wx", function (err, fd) {
...
module.exports.createReport = function(submitterUid:string, reportedUid:string, message:string):void{
...
module.exports.getUserData = function(uid: number):Promise<User>{
    return new Promise((resolve, reject) => {
        db.get(`SELECT * FROM users WHERE uid=${uid}`, (err: Error, row: any) => {
            if(err){
                logger.error(`DB: Cant get User-Data from ${uid} - err: ${err.message}`);
                return reject(err);
            }
            if(row && row.uid){
                resolve(new User(row.uid, row.lastname, row.firstname, row.birthday, row.email, row.password, row.sex, row.city, row.description, row.picture, row.secAnswer));
            } else {
                reject({});
            }
        })
...
module.exports.createNewUser = function(userData: User):void{
    db.run(`INSERT INTO users(lastname, firstname, birthday, email, password, sex, city, description, picture, secAnswer)
            VALUES
...
module.exports.updateUserData = function(uid: number, userData: User):void{
...
module.exports.changeUserPassword = function(email: string, password: string):void{
...
module.exports.validateSecurityAnswer = function(email: string, answer: string):Promise<boolean>{
    return new Promise((resolve, reject) => {
        doesEmailExist(email).then((res:boolean) =>{
            if(res){
                db.get(`SELECT * FROM users WHERE email = $email`, {$email: email}, (err: Error, row: any) => {
                        if(err){
...
function doesEmailExist(email:string):Promise<boolean>{
    return new Promise((resolve, reject) => {
        db.all(`SELECT * FROM users WHERE email = $email`, {$email: email}, (err: Error, rows: any) => {
            if(err){
...
function doesUidExist(uid:string):Promise<boolean>{
    return new Promise((resolve, reject) => {
        db.all(`SELECT * FROM users WHERE uid = $uid`, {$uid: uid}, (err: Error, rows: any) => {
            if(err){
...
module.exports.getAllUsers = function():Promise<User[]>{
    return new Promise((resolve, reject) => {
        db.all(`SELECT * FROM users`, (err: Error, rows: any) => {
            ...
            for(var row in rows){
                users.push(new User(rows[row].uid, rows[row].lastname, rows[row].firstname, rows[row].birthday, rows[row].email, rows[row].password, rows[row].sex, rows[row].city, rows[row].description, rows[row].picture, rows[row].secAnswer));
            }
            shuffleArray(users);
            resolve(users);
        })
...
module.exports.getAllReports = function():Promise<Report[]>{
    return new Promise((resolve, reject) => {
        db.all(`SELECT * FROM reports`, (err: Error, rows: any) => {
            if(err){
            ...
            for(var row in rows){ //submitterUid, reportedUid, message
                reports.push(new Report(rows[row].submitterUid, rows[row].reportedUid, rows[row].message));
            }
            resolve(reports);
        })
    })
...
module.exports.getAllUnmatchedUsers =function(uid:number):Promise<User[]>{
    return new Promise((resolve, reject) => {
        getUidsfromUserMatches(uid).then((uids: number[])=>{
            //query zusammenbauen
            var query_str = "";
            ...
            db.all(`SELECT * FROM users WHERE uid NOT IN (${query_str})`, (err: Error, rows: any) => {
                if(err){
                    logger.error(`DB: Cant getAllUnmatchedUsers - err:${err.message}`);
                    reject(err);
                }

                var unmatched_users: User[] = [];
                for(var row in rows){
                    if(rows[row].uid == uid) continue;
                    unmatched_users.push(new User(rows[row].uid, rows[row].lastname, rows[row].firstname, rows[row].birthday, rows[row].email, rows[row].password, rows[row].sex, rows[row].city, rows[row].description, rows[row].picture, rows[row].secAnswer));
                }
                shuffleArray(unmatched_users);
 ...
 module.exports.getAllMatchedUsers = function(uid:number):Promise<User[]>{
    return new Promise((resolve, reject) => {
        getUidsfromUserMatches(uid).then((uids: number[])=>{
            //query zusammenbauen
            var query_str = "";
            ...
            //Daten der Nutzer abfragen
            db.all(`SELECT * FROM users WHERE uid IN (${query_str})`, (err: Error, rows: any) => {
                if(err){
                    logger.error(`DB: Cant getAllMatchedUsers - err:${err.message}`);
                    reject(err);
                }

                var matched_users: User[] = [];
                for(var row in rows){
                    if(rows[row].uid == uid) continue;
                    matched_users.push(new User(rows[row].uid, rows[row].lastname, rows[row].firstname, rows[row].birthday, rows[row].email, rows[row].password, rows[row].sex, rows[row].city, rows[row].description, rows[row].picture, rows[row].secAnswer));
                }
...
function getUidsfromUserMatches(uid:number):Promise<number[]>{
    return new Promise((resolve, reject) => {
        db.all(`SELECT * FROM matches WHERE uid1=${uid} OR uid2=${uid}`, (err: Error, rows: any) => {
                if(err) {
...
module.exports.getChatfromUsers = function(uid1:number, uid2:number, actualUserId:number):Promise<Message[]>{
    return new Promise((resolve, reject) => {
        //Dateipfad zum Chat
        var filePath:string = `chats/chat_${uid1}-${uid2}`
        ...
        fs.readFile(filePath, 'utf8', (err, data:any) => {
            ...
            if(err){
                ...
                for(var i=0; i < lines.length; i++){
                    if(lines[i].length > 0){
                        var fromCurrentUser:boolean;
                        var senderId:number = Number(lines[i].slice(0, 1));
                        if(actualUserId != uid1 && actualUserId != uid2){
                            if(senderId == uid1){
                                msgs.push(new Message(lines[i], true));
                            } else {
                                msgs.push(new Message(lines[i], false));
                            }
                        } else {
                            fromCurrentUser = senderId == actualUserId ? true : false;
                            msgs.push(new Message(lines[i], fromCurrentUser));
                        }
                    }
                }
                resolve(msgs);
...
module.exports.writeMessageIntoChat = function(message: String, senderId: number, otherUsersId: number){

    var msg:string = senderId+":"+message+"\r\n";
    var biggerUid:number = Math.max(senderId, otherUsersId);
    var smallerUid:number = Math.min(senderId, otherUsersId);

    const writeSteam = fs.createWriteStream(`chats/chat_${smallerUid}-${biggerUid}`, {
...
module.exports.closeDbConnection = () => {
    db.close((err: Error) => {
        ...
        else return logger.info('db connection closed');
    }) 
}
